...
% Example 2 - Backward TRF (stimulus reconstruction)
%
...
% runs a typical backward TRF analysis.
%
% Note:
...
tmin = -50;
tmax = 300;
lambdas = [1e-2,1e0,1e2,1e4]; % small set of lambdas (quick)
...
windowSize = 5; % window-size for the match-vs-mismatch decoding. Good 
                % classification scores means that the reconstructed stim
                % could be identified vs. a random mismatch (circularshift)
                % on 3-second long chunks of neural data
skipNeuralChannels = 8; % downsampling factor (e.g., selecting only one
                        % out of eight channels). This should not be done
                        % typically. It's just for making the tutorial
...
% TRF
clear rAll rAllElec modelAll accMM dMM
...
for sub = 1:nSubs
    ...
    % Reducing the number of channels for computational reasons (there are
    % better ways to reduce the dimensionality of course, such as DSS/JD
    % and MCCA. Or you can keep all channels.
    % Note: This is not the typical approach! It is just for making this
    % computation tractable at the CNSP workshop.
    for tr = 1:length(neural.data)
        neural.data{1,tr} = neural.data{1,tr}(:,1:skipNeuralChannels:end);
    end
    neural.chanlocs = neural.chanlocs(1:skipNeuralChannels:end);
    ...
    % Calculating match-vs-mismatch score
    statsMM = mTRFmatchVSmismatch(stimFeature,neural,dirTRF,tmin,tmax,lambdas(bestLambda),windowSize);
    accMM(sub,:) = statsMM.acc;
    dMM(sub,:)   = statsMM.d;
    
    if dirTRF == 1
        mTRF_plotForwardTRF(neural,modelAll,rAllElec);
    elseif dirTRF == -1
        mTRF_plotBackwardTRF(neural,modelAll,rAllElec,mean(accMM,2));
    end
    
    disp(['Mean r = ',num2str(mean(rAll))])
    disp(['Match-vs-mismatch classification accuracy: ',num2str(mean(accMM(sub,:),2))])
    disp(['Match-vs-mismatch d-prime: ',num2str(mean(dMM(sub,:),2))])
