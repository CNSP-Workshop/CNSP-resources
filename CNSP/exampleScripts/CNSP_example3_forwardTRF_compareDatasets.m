...
% Example 3 - Compare forward TRFs between datasets
%
...
datasets2compare = {'../datasets/LalorNatSpeech/','../datasets/LalorNatSpeechReverse/','../datasets/AliceSpeech/'};
downFsAll = [64,64,50];
dataCNDSubfolder = 'dataCND/';
toPreprocess = [0,0,0]; % 1 if the dataset should be preprocessed
                        % 0 if the preprocessing was run already
...
for dataIdx = 1:length(datasets2compare)
    dataMainFolder = datasets2compare{dataIdx};
    downFs = downFsAll(dataIdx); % Hz. *** fs/downFs must be an integer value ***
    ...
    for sub = 1:nSubs
        ...
        % Ensuring that stim and neural have the same length
        [stimFeature,neural] = cndCutSameLength(stimFeature,neural);
