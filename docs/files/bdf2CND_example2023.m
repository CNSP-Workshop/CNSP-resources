...
trigCode_audioStart = 1:40; % all trigger codes corresponding to the
                            % start of a trial
nAudioFiles = 20;           % number of audio files (usually called
                            % audio1.wav, audio2.wav, etc
nCond = 2;                  % number of conditions

% Preprocessing parameters (a typical scenario including several audio
% segments, but no conditions e.g., audio-book listening)
% trigCode_audioStart = 1:20; % all trigger codes corresponding to the
%                             % start of a trial
% nAudioFiles = 20;           % number of audio files
% nCond = 1;                  % number of conditions
...
for iiSub = subs                  % for each subject
    ...
    % Set bdf filename
    bdfFilename = [rawEEGPath,'./sub',num2str(iiSub),'.bdf']; % e.g., sub1.bdf
    % Load bdf
    ...
    trigsCode = trigs(allTrigsSample);
    ...
    neural.data = cell(1,length(fileList));
    neural.extChan{1}.data = cell(1,length(fileList));
    neural.extChan{2}.data = cell(1,length(fileList));
    ...
    for iiSegment = 1:length(fileList)
        % Calculating which audio file and condition were used in this segment
        iiAudio = mod(trigsCode(iiSegment),nAudioFiles); % e.g., trig 21 indicates audio 1 cond 2,
                                                         % when only 20 audio files are involved
        iiCond = floor((trigsCode(iiSegment)-1)/nAudioFiles);

        % Calculating samples for the chunking
        stimStart = trigsSample(iiSegment);
        stimEnd = stimStart + audioLengths(iiAudio);
